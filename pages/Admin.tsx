              ...
              <div className="bg-white p-8 rounded-[2rem] shadow-xl border border-navy/5 grid grid-cols-1 md:grid-cols-4 gap-6">
                <div>
                  ...
                  <select 
                    ...
                    value={siteConfig.galleryLayoutStyle || 'grid'}
                    onChange={e => saveGallery(undefined, { galleryLayoutStyle: e.target.value as any })}
                  >
                ...
                <div>
                  ...
                  <select 
                    className="w-full p-3 bg-cream/30 border-none rounded-xl font-bold text-navy"
                    value={siteConfig.galleryColumns || 3}
                    onChange={e => saveGallery(undefined, { galleryColumns: Number(e.target.value) as any })}
                  >
                ...
                <div>
                  <label className="text-[10px] font-black text-navy/40 uppercase tracking-widest mb-1 block">Gallery Spacing (px)</label>
                  <input 
                    type="number"
                    className="w-full p-3 bg-cream/30 border-none rounded-xl font-bold text-navy"
                    value={siteConfig.gallerySpacing || 4}
                    onChange={e => saveGallery(undefined, { gallerySpacing: Number(e.target.value) })}
                  />
                </div>
          ...
          <div className="max-w-xl mx-auto space-y-8">
            <div className="bg-white p-10 rounded-[3rem] shadow-2xl border border-navy/5 text-center">
              ...
              <div className="flex items-center justify-center gap-6 mb-12">
                <span className={`text-xs font-black uppercase tracking-[0.2em] ${!siteConfig.isMaintenance ? 'text-green-600' : 'text-navy/20'}`}>Live</span>
                <button onClick={() => setSiteConfig(prev => prev ? {...prev, isMaintenance: !prev.isMaintenance} : null)} className={`w-20 h-10 rounded-full relative transition-all duration-500 shadow-inner ${siteConfig.isMaintenance ? 'bg-burntOrange' : 'bg-green-500'}`}>
                  <div className={`absolute top-1 w-8 h-8 bg-white rounded-full transition-all duration-500 shadow-lg ${siteConfig.isMaintenance ? 'left-11' : 'left-1'}`}></div>
                </button>
                <span className={`text-xs font-black uppercase tracking-[0.2em] ${siteConfig.isMaintenance ? 'text-burntOrange' : 'text-navy/20'}`}>Maintenance</span>
              </div>
