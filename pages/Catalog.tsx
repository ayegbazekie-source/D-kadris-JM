...
const Catalog: React.FC = () => {
  ...
  const fetchProducts = async (pageNum: number, isNewFilter = false) => {
    setLoading(true);
    try {
      const response = await apiService.getCatalogs(pageNum, 12);
      if (isNewFilter) {
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        setSiteConfig(liveConfig.siteSettings);
        // If we want to strictly follow "render from live config state"
        // we could set products from liveConfig.catalogs
        // but since we have pagination, we'll keep the fetchProducts but ensure it's live
        setProducts(liveConfig.catalogs || []);
        setHasMore(false); // Since we loaded all from liveConfig
      } catch (err) {
  ...
  const getImageSizeClass = (size?: Product['imageSize']) => {
    if (!size) return 'aspect-[3/4]';
    if (typeof size === 'string') {
      switch (size) {
        case 'small': return 'aspect-square max-w-[200px] mx-auto';
        case 'large': return 'aspect-[2/3]';
        default: return 'aspect-[3/4]';
      }
    }
    return ''; // Custom size handled by style prop
  };

  const getImageStyle = (size?: Product['imageSize']) => {
  ...
  const displayProducts = useMemo(() => {
    return products
      ...
      .filter(p => filter === 'all' || (p.categories && p.categories.includes(filter as any)) || p.category === filter)
      .sort((a, b) => {
  ...
  const submitOrder = async () => {
    ...
    const newOrder: Order = {
      ...
      measurements,
      timestamp: Date.now(),
      customerEmail: 'customer@whatsapp.com', // Placeholder, ideally captured in a form
  ...
  return (
    <div className="min-h-screen pt-24 pb-20 px-4 md:px-6 bg-cream">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="sticky top-20 z-40 bg-cream/80 backdrop-blur-md py-4 mb-12 flex flex-col md:flex-row justify-between items-center gap-4 border-y border-navy/5 px-4 overflow-x-auto">
          <div className="flex gap-2 bg-white/50 p-1 rounded-2xl border min-w-max">
            {(['all', 'men', 'women', 'unisex', 'jeans', 'shorts', 'jackets', 'custom'] as const).map(cat => (
              <button
                key={cat}
                onClick={() => setFilter(cat)}
                className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${filter === cat ? 'bg-navy text-gold shadow-md' : 'text-navy/60'}`}
              >
                {cat}
              </button>
            ))}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {displayProducts.map(product => (
            <div key={product.id} className="bg-white rounded-[2rem] overflow-hidden shadow-xl hover:-translate-y-1 transition-all border border-navy/5 flex flex-col">
              <div className={`${getImageSizeClass(product.imageSize)} overflow-hidden bg-cream/20`} style={getImageStyle(product.imageSize)}>
                <img 
                  src={product.image} 
        ...
        {displayProducts.length === 0 && !loading && (
          <div className="text-center py-20">
            <p className="text-navy/40 font-bold uppercase tracking-widest">No products available yet.</p>
          </div>
        )}

        {loading && (
          <div className="flex justify-center py-12">
            <div className="w-10 h-10 border-4 border-gold border-t-navy rounded-full animate-spin"></div>
          </div>
        )}
        ...
        {hasMore && !loading && (
          <div className="mt-16 text-center">
            <button 
              onClick={loadMore}
              className="bg-white text-navy border-2 border-navy px-12 py-4 rounded-2xl font-black uppercase tracking-widest text-xs hover:bg-navy hover:text-gold transition-all shadow-md"
            >
              Load More Designs
            </button>
          </div>
